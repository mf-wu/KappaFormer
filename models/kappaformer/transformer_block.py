...
class SO2EquivariantGraphAttention(torch.nn.Module):
    ...
    def __init__(
        ...
        if not self.use_s2_act_attn: # Go
        ...
        # Physics
        # self.alpha_act_phy = SmoothLeakyReLU()
        self.head_dim = self.sphere_channels // self.num_heads
        self.MAX_ATOMS = max_num_elements
        self.num_distance_basis = num_distance_basis
        self.pe_proj = nn.Linear(self.num_distance_basis, self.sphere_channels)
        self.pe_mul_ = nn.Embedding(self.MAX_ATOMS*self.MAX_ATOMS, self.sphere_channels)
        self.pe_bais_ = nn.Embedding(self.MAX_ATOMS*self.MAX_ATOMS, self.sphere_channels)
        nn.init.constant_(self.pe_bais_.weight, 0)
        nn.init.constant_(self.pe_mul_.weight, 1)
    ...
    def forward(
        ...
        if self.use_s2_act_attn:
            ...
            x_message, x_0_extra = self.so2_conv_1(x_message, x_edge) # Go
        ...
        if self.use_s2_act_attn:
            ...
            x_0_alpha = self.alpha_norm(x_0_alpha) # LayerNorm
            x_0_alpha = self.alpha_act(x_0_alpha) # LeakyReLU
            alpha = torch.einsum('bik, ik -> bi', x_0_alpha, self.alpha_dot) # [n_edges, h_heads]
        
        # Physics bias
        src, dst = edge_index 
        E = edge_index.size(1)
        edge_pairatom_type = atomic_numbers[src] * self.MAX_ATOMS + atomic_numbers[dst]
        pe_w = self.pe_mul_(edge_pairatom_type)
        pe_b = self.pe_bais_(edge_pairatom_type)
        pe_score = self.pe_proj(edge_distance) * pe_w + pe_b
        pe_score = pe_score.view(E,self.num_heads,self.head_dim).sum(-1)
        # pe_score = self.alpha_act_phy(pe_score)

        # Add physics socre
        alpha_all = pe_score + alpha

        # weights
        alpha = torch_geometric.utils.softmax(alpha_all, edge_index[1]) 
