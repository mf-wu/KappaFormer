...
class Kappaformer(BaseModel):
    ...
    def forward(self, data):
        ...
        # global gate
        scores_harm = self.pool_harm.gate_nn(x_moe_harm)
        gate_harm = softmax(scores_harm, data.batch).squeeze(-1) 
        ...
        if self.enable_kappa:
            ...
            # global gate
            scores_anharm = self.pool_harm.gate_nn(x_moe_anhamrm)
            gate_anharm = softmax(scores_anharm, data.batch).squeeze(-1) 
            print("Harm:", y_harm)
            print("Anharm:", y_anharm)
            if self.return_embedding:
                return b,g,log_kappa,x_pool_harm,x_pool_anharm
            else:
                return b,g,log_kappa, gate_harm, gate_anharm
        else:
            if self.return_embedding:
